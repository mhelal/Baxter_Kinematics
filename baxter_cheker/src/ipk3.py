...
def inverse_kinematics(desired_position, desired_orientation, position_weight, orientation_weight):
    # Initial guess for joint angles
    theta_initial = np.zeros(7)
    
    # Solve for joint angles using gradient descent
    result = scipy.optimize.least_squares(cost_function, theta_initial, args=(desired_position, desired_orientation, position_weight, orientation_weight), method='trf')
    return result.x
...
if __name__ == '__main__':
    ...
    # Weights for position and orientation errors
    position_weight = 1.0
    orientation_weight = 0.1

    # Solve for joint angles
    theta = inverse_kinematics(desired_position, desired_orientation, position_weight, orientation_weight)

    # Print results
    print("Joint angles:")
    print(theta)
    print(cost_function(theta, desired_position, desired_orientation, position_weight, orientation_weight))
